...
icon = Image.open("Bogh_transparent.png")
...
SmallestAcceptableTime = 0 # in seconds

uploaded_file =st.file_uploader("Upload a CSV file", type=['csv'])
if uploaded_file is None:
    df = pd.DataFrame([0])
    st.subheader('To start, please upload a CSV file')
if uploaded_file is not None:
    ...
    # Finds the right delimiter
    if len(columns) <= 2:
        uploaded_file.seek(0)  # Reset the file pointer to the beginning
        df = pd.read_csv(uploaded_file, delimiter=';')
        columns = df.columns.tolist()

## This is specific for cubing data, but will make a column for all datasets uploaded. Might consider making it optional.
...
def ConverCSVtimeToSeconds(time):
    if 'Time' in df.columns:
        ...
        #Removes times that didnt finish. They are in parentheses, so the first four elements and the last are removed.
        if time[0:3] == "DNF":
            time = time[4:-1]
...
with col1:
        ...
        max_x_value = st.slider("x-values", 
                                value = [int(min(df[x_axis])), int(max(df[x_axis])+1)], 
                                min_value = int(min(df[x_axis])), 
                                max_value = int(max(df[x_axis])+1))
        max_y_value = st.slider("y-values", 
                                value = [int(min(df[y_axis])), int(max(df[y_axis])+1)], 
                                min_value = int(min(df[y_axis])) if int(min(df[y_axis])) < 0 else 0, 
                                max_value = int(max(df[y_axis])+1))

x_values = df[x_axis][max_x_value[0]:max_x_value[1]]
#if y_axis != "None":
    #y_values = df[y_axis][max_y_value[0]:max_y_value[1]]
y_values = df[y_axis][max_x_value[0]:max_x_value[1]]
...
def plots():
    if plot_type == 'Line Plot':
        ...
        sns.countplot(x=x_values, ax=ax, color='blue')
        y_axis = 'Count'

# Generate the plot
if col3.button('Generate Plot'):
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.set_ylim([max_y_value[0], max_y_value[1]])
...
# Make the sidebar list
Sidebar_list = [x_axis, y_axis]

# Define SelectedAxis to be used in the sidebar
def SelectedAxis(Axis_values):
    st.text("0. kvartil: " + str(np.nanquantile(Axis_values,0/4)))
    st.text("1. kvartil: " + str(np.nanquantile(Axis_values,1/4)))
    st.text("2. kvartil: " + str(np.nanquantile(Axis_values,2/4)))
    st.text("3. kvartil: " + str(np.nanquantile(Axis_values,3/4)))
    st.text("4. kvartil: " + str(np.nanquantile(Axis_values,4/4)))
    st.text("Mean: " + "{:.2f}".format(np.nanmean(Axis_values)))
    st.text("Variance: " + "{:.2f}".format((np.nanquantile(Axis_values,4/4)) - (np.nanquantile(Axis_values,0/4))))
    st.text("Standard deviation: " +"{:.2f}".format((np.nanstd(Axis_values))))

# Sidebar with statistics
with st.sidebar:
    st.title('Statistics')
    Sidebar_type = st.selectbox('Select an axis to display statistics', options=Sidebar_list)
    if Sidebar_type == x_axis:
        SelectedAxis(x_values)
    elif Sidebar_type == y_axis:
        SelectedAxis(y_values)


st.markdown('<div style="font-size: 8px; text-align: center">Made by CC. Dam, Magnus Grøndahl & Nilias Olsen</div>', 
